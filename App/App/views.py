...
def usercheck_conect(uname,password,cnx):
  if uname=="NULL":
    return "False"
  Q1=("SELECT * FROM `a_final_users_table` WHERE `uname` LIKE %s AND `hashword` LIKE %s")
...
def add_user(uname,password,email,cnx,return_var_type):
  ...
  if newname!=uname:
    return "No _"
  sql1 = "SELECT * FROM `a_final_users_table` WHERE `uname` LIKE %s";
  cursor = cnx.cursor()
  tuple1 = (uname)
  ...
  if counter==0:
    sql="INSERT INTO `a_final_users_table` (`hashword`, `uname`, `email`,`time`) VALUES (%s, %s, %s, CURRENT_TIMESTAMP);";
...
def add_post(uname,password,tital,text,body,photo,catagoy,catagoy_2,iframe,cnx,return_var_type):
  ...
  sql="INSERT INTO `a_final_posts` (`uname`, `text`, `body`, `tital`, `time`, `photo`, `iframe`, `catagoy`, `catagoy_2`, `postkey`) VALUES (%s, %s, %s, %s, CURRENT_TIMESTAMP, %s, %s, %s, %s, %s);";
...
def check_priavate(key,private,cnx):
  if private=="":
    return "True"
  sql="SELECT * FROM `a_final_posts` WHERE `uname` LIKE 'addmin' AND `catagoy_2` LIKE %s AND `postkey` LIKE %s;"
  tuple1 = (private,key)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  counter=0
...
def getpost(key,usekkey,cnx,return_var_type):
  Q1=("SELECT `uname`,`text`,`body`,`tital`,`time`,`photo`,`iframe`,`catagoy`,`catagoy_2` FROM `a_final_posts` WHERE `postkey` LIKE %s;")
  tuple1= (key)
  cursor = cnx.cursor()
  cursor.execute(Q1,tuple1)
  counter=0
...
def add_ledgure(uname,password,email,hashword,Ledgure,cnx,return_var_type):
  ...
  try:
    sql="INSERT INTO `a_final_Ledgur` (`Ledgurename`, `Ledgurepassword`, `email`, `time`) VALUES (%s, %s, %s, CURRENT_TIMESTAMP);";
...
def add_key(ledgure,password,email,message,key_message,keyfroward,cnx,return_var_type):
  ...
  Q1=("SELECT `email` FROM `a_final_Ledgur` WHERE `Ledgurename` LIKE %s and `Ledgurepassword` LIKE %s;")
...
def change_post(user,password,key,text,cnx,return_var_type):
  sql="SELECT `uname` FROM `a_final_posts` WHERE `postkey` LIKE %s;"
  tuple1= (key)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
  ...
  if letsgo=="False":
    ...
    return json.dumps(dictionary, indent = 5)
  sql ="UPDATE `a_final_posts` SET `text` = %s WHERE `a_final_posts`.`postkey` = %s; "
  tuple1= (text,key)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
...
def change_key(key,name,newkey,cnx ,return_var_type):
  sql = "SELECT `hash`,`ledgername`,`forward`,`key_message` FROM `a_final_Ledgur_keys` WHERE `entery_name` LIKE %s AND `solution` LIKE 'key';"
  tuple1 = (name)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
  ...
  if hashlib.sha256(key.encode()).hexdigest()!=hashs:
    ...
    return json.dumps(dictionary, indent = 5)
  #change key case
  sql ="UPDATE `a_final_Ledgur_keys` SET `solution` = %s WHERE `entery_name` = %s; "
  tuple1 = (key,name)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
  ...
  myrandom = get_random_string(128)
  post_id = hashlib.sha256(myrandom.encode()).hexdigest()
  sql="INSERT INTO `a_final_Ledgur_keys` (`entery_name`, `ledgername`, `hash`, `solution`, `email`,`time`,`forward`,`key_message`) VALUES (%s, %s, %s, 'key', 'none', CURRENT_TIMESTAMP, %s, %s);";
...
def check_key(name,cnx,return_var_type):
  sql = "SELECT `hash`,`ledgername`,`solution` FROM `a_final_Ledgur_keys` WHERE `entery_name` LIKE %s;"
  tuple1 = (name)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
...
def rm_key(name,key,message,cnx,return_var_type):
  sql = "SELECT `hash`,`ledgername`,`forward`,`key_message`,`email` FROM `a_final_Ledgur_keys` WHERE `entery_name` LIKE %s AND `solution` LIKE 'key';"
  tuple1 = (name)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
  ...
  myrandom = get_random_string(128)
  post_id = hashlib.sha256(myrandom.encode()).hexdigest()
  sql="INSERT INTO `a_final_posts` (`uname`, `text`, `body`, `tital`, `time`, `photo`, `iframe`, `catagoy`, `catagoy_2`, `postkey`) VALUES ('addmin', '', '', '', CURRENT_TIMESTAMP, '', '', '', %s, %s);";
...
def add_template(username,password,template_name,template,replace,cnx,return_var_type):
  ...
  try:
    sql=  "INSERT INTO `a_final_template7` (`user`, `usertemplate_name`, `template`, `time`) VALUES (%s, %s, %s, CURRENT_TIMESTAMP); "
...
def make_setion(myid,cnx,make):
  sql="INSERT INTO `a_final_posts` (`uname`, `text`, `body`, `tital`, `time`, `photo`, `iframe`, `catagoy`, `catagoy_2`, `postkey`) VALUES ('', '', '', '', CURRENT_TIMESTAMP, '', '', %s, '', %s);";
  tuple1 = (make,myid)
  cursor = cnx.cursor()
  cursor.execute(sql,tuple1)
  cnx.commit()
...
def return_template2(usertemplate_name,var1,setion,setion2,rep,cnx):
  path=path_getter()+"/doit"
  sql = "SELECT `template` FROM `a_final_template7` WHERE `usertemplate_name` LIKE %s"
